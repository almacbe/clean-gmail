...
const makeMessageGetResponse = (
  ...
  data: {
    ...
    payload: {
      headers: [
        { name: 'From', value: options.sender ?? 'social@example.com' },
        { name: 'Subject', value: options.subject ?? 'Test Social' },
        {
          name: 'Date',
...
describe('GmailSocialAdapter', () => {
  ...
  it('calls messages.list with q: "category:social"', async () => {
    mockMessagesList.mockResolvedValue(makeMessageListResponse([]));

    const adapter = new GmailSocialAdapter();
    await adapter.scanSocial('test-token');

    expect(mockMessagesList).toHaveBeenCalledWith(
      expect.objectContaining({ q: 'category:social', userId: 'me' }),
    );
  });
  ...
  it('retries on 429 and succeeds on second attempt', async () => {
    ...
    const adapter = new GmailSocialAdapter(noopDelay);
    const result = await adapter.scanSocial('test-token');
  ...
  it('exhausts retries and throws after 3 attempts', async () => {
    ...
    await expect(adapter.scanSocial('test-token')).rejects.toThrow(
      'Rate limited',
    );
  ...
  it('sets OAuth2 credentials with access token', async () => {
    ...
    const adapter = new GmailSocialAdapter();
    await adapter.scanSocial('my-access-token');
