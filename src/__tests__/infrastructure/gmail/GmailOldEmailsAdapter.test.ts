...
const makeMessageGetResponse = (
  ...
  data: {
    ...
    payload: {
      headers: [
        { name: 'From', value: options.sender ?? 'old@example.com' },
        { name: 'Subject', value: options.subject ?? 'Test Old Email' },
        {
          name: 'Date',
          value: options.date ?? 'Mon, 15 Jan 2020 10:00:00 +0000',
        },
      ],
    },
  },
...
describe('GmailOldEmailsAdapter', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it('sends older_than:1y for olderThan="1y"', async () => {
    mockMessagesList.mockResolvedValue(makeMessageListResponse([]));

    const adapter = new GmailOldEmailsAdapter();
    await adapter.scanOldEmails('test-token', '1y');

    expect(mockMessagesList).toHaveBeenCalledWith(
      expect.objectContaining({ q: 'older_than:1y', userId: 'me' }),
    );
  });

  it('sends older_than:6m for olderThan="6m"', async () => {
    mockMessagesList.mockResolvedValue(makeMessageListResponse([]));

    const adapter = new GmailOldEmailsAdapter();
    await adapter.scanOldEmails('test-token', '6m');

    expect(mockMessagesList).toHaveBeenCalledWith(
      expect.objectContaining({ q: 'older_than:6m', userId: 'me' }),
    );
  });

  it('sends older_than:2y for olderThan="2y"', async () => {
    mockMessagesList.mockResolvedValue(makeMessageListResponse([]));

    const adapter = new GmailOldEmailsAdapter();
    await adapter.scanOldEmails('test-token', '2y');

    expect(mockMessagesList).toHaveBeenCalledWith(
      expect.objectContaining({ q: 'older_than:2y', userId: 'me' }),
    );
  });

  it('sends older_than:5y for olderThan="5y"', async () => {
    mockMessagesList.mockResolvedValue(makeMessageListResponse([]));

    const adapter = new GmailOldEmailsAdapter();
    await adapter.scanOldEmails('test-token', '5y');

    expect(mockMessagesList).toHaveBeenCalledWith(
      expect.objectContaining({ q: 'older_than:5y', userId: 'me' }),
    );
  });
  ...
  it('retries on 429 and succeeds on second attempt', async () => {
    ...
    const adapter = new GmailOldEmailsAdapter(noopDelay);
    const result = await adapter.scanOldEmails('test-token', '1y');
  ...
  it('exhausts retries and throws after 3 attempts', async () => {
    ...
    await expect(adapter.scanOldEmails('test-token', '1y')).rejects.toThrow(
      'Rate limited',
    );
  ...
  it('sets OAuth2 credentials with access token', async () => {
    ...
    const adapter = new GmailOldEmailsAdapter();
    await adapter.scanOldEmails('my-access-token', '5y');
