...
export class GmailLargeEmailAdapter implements LargeEmailScanner {
  ...
  async scanLargeEmails(accessToken: string): Promise<EmailMetadata[]> {
    const oauth2Client = new google.auth.OAuth2();
    oauth2Client.setCredentials({ access_token: accessToken });

    const gmail = google.gmail({ version: 'v1', auth: oauth2Client });

    // Step 1: Paginate messages.list to collect all IDs
